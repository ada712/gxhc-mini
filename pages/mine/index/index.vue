<template>
	<view class="center-wripper">
		<view class="top-module">
			...
			<view class="login-box" v-if="isLogin">
				<view class="row">
					<view class="row-left" @click="goPersonInfoPage">
						<view class="head">
							<image :src="userInfo.avatarUrl" class="img-head" v-if="userInfo.avatarUrl" />
							<image :src="imgPath+'/mine/default-head.png'" class="img-head" v-else />
						</view>
						<view class="info">
							<view class="nickname">{{userInfo.nickname}}</view>
							<view class="mark-row">
								<view class="identity-box" v-if="identityLabel">{{identityLabel}}</view>
								<view class="jyz" v-if="showCampusModule">经验值{{experienceValue}}</view>
							</view>
						</view>
					</view>
					<view class="row-right" @click="goMessageList">
						<view class="message-box">
							<image :src="imgPath+'/mine/icon-lingdang.png'" class="icon-ld" />
							消息
						</view>
					</view>
				</view>
			...
			<view class="login-box" v-if="!isLogin">
				<view class="row" @click="goLoginPage">
					<view class="row-left">
						<view class="head">
							<image :src="imgPath+'/mine/default-head.png'" class="img-head" />
						</view>
		...
		<view class="content-module">
			<image :src="imgPath+'/mine/banner240.png'" class="img-banner" @click="goGxhcIntroduce" />

			<view class="message-row" v-if="isLogin && messageTips">
				<image :src="imgPath+'/mine/icon_tongzhi.png'" class="icon-noice" />
				<view class="ft28 tips">您的项目已经通过审核了，详情通过功能模块，项目进展了解更多</view>
			</view>

			<view class="same-box" v-if="isLogin && showCampusModule">
				<view class="same-menus">
					<view class="item-menu" v-for="(item,key) in campusMenus" :key="key" @click="handleGoPage(item)">
			...
			<view class="same-box" v-if="isLogin && (isCampusManage || isAdmin)">
				...
				<view class="same-menus">
					<view class="item-menu" v-for="(item,key) in campusManageMenus" :key="key" @click="handleGoPage(item)">
						<image :src="item.icon" class="icon-color-menu" />
						<view class="menu-name">{{item.name}}</view>
					</view>
				</view>
			...
			<view class="common-box">
				<view class="row common-row" v-if="isLogin && (isCampusManage || isAdmin)" @click="goAdminCenterPage">
					<view class="left">
						<image :src="imgPath+'/mine/icon_khzx.png'" class="icon_brief_menu" />
						<text class="ft32">管理员操作中心</text>
					</view>
					<view class="right">
						<image :src="imgPath+'/icons/icon-gray-right2.png'" class="icon_right_arrow" />
					</view>
				</view>
				...
				<view class="row common-row" @click="goProtocolPage">
					<view class="left">
						<image :src="imgPath+'/mine/icon_xysm.png'" class="icon_brief_menu" />
				...
				<button class="row common-row" open-type="feedback">
					<view class="left">
						<image :src="imgPath+'/mine/icon_fkjy.png'" class="icon_brief_menu" />
				...
				<view class="row common-row" @click="goCustomerPage">
					<view class="left">
						<image :src="imgPath+'/mine/icon_zxkf.png'" class="icon_brief_menu" />
...
<script>
	...
	export default {
		...
		methods: {
			...
			goGxhcIntroduce() {
				this.navigateToPage('https://mp.weixin.qq.com/s/kCbiycoxR5xv5EPjwl7zaA', true)
			},
			goProtocolPage() {
					...
					.info {
						...
						.mark-row {
							...
							.identity-box {
								...
								box-sizing: border-box;
								padding: 6rpx 10rpx;
								background: linear-gradient(316deg, #2D65B8 0%, #668BD7 61%, #6493EC 100%);
								font-size: 18rpx;
								color: #FFFFFF;
								line-height: 24rpx;
								display: flex;
