...
const util = {
  ...
  Tips: function(opt, to_url) {
    ...
    let title = opt.title || "", icon = opt.icon || "none", endtime = opt.endtime || 2e3, success = opt.success;
    if (title)
    ...
    common_vendor.index.getImageInfo({
      ...
      success: function(res) {
        ...
        for (let m = 0; m < contentArray.length; m++) {
          if (m) {
            ctx.fillText(contentArray[m], 50, 1e3 + contentHh * m + 18, 1100);
          } else {
            ctx.fillText(contentArray[m], 50, 1e3 + contentHh * m, 1100);
          }
        }
        ctx.setTextAlign("left");
        ctx.setFontSize(72);
        ...
        if (ot_price) {
          ...
          var underline = function(ctx2, text, x, y, size, color, thickness, offset) {
            var width = ctx2.measureText(text).width;
            switch (ctx2.textAlign) {
              case "center":
                x -= width / 2;
                break;
              case "right":
        ...
        ctx.draw(true, function() {
          common_vendor.index.canvasToTempFilePath({
            ...
            success: function(res2) {
              ...
              successFn && successFn(res2.tempFilePath);
            }
          });
        });
      },
  ...
  userPosterCanvas: function(arr2, nickname, sitename, index, w, h, successFn) {
    let that = this;
    const ctx = common_vendor.index.createCanvasContext("myCanvas" + index);
    ctx.clearRect(0, 0, 0, 0);
    common_vendor.index.getImageInfo({
      src: arr2[1],
      success: function(res) {
    ...
    common_vendor.index.chooseImage({
      ...
      success: function(res) {
        ...
        common_vendor.index.uploadFile({
          url: config_app.HTTP_REQUEST_URL + "/api/" + uploadUrl,
          ...
          header: {
            ...
            [config_app.TOKENNAME]: "Bearer " + store_index.store.state.app.token
          },
          success: function(res2) {
            common_vendor.index.hideLoading();
      ...
      success: function(res) {
        common_vendor.index.getImageInfo({
          ...
          success(ress) {
            ...
            let canvas = common_vendor.index.createCanvasContext("canvas");
  ...
  $L: {
    ...
    doGetLocation() {
      common_vendor.index.getLocation({
        success: (res) => {
          common_vendor.index.removeStorageSync("CACHE_LONGITUDE");
          common_vendor.index.removeStorageSync("CACHE_LATITUDE");
          common_vendor.index.setStorageSync("CACHE_LONGITUDE", res.longitude);
          common_vendor.index.setStorageSync("CACHE_LATITUDE", res.latitude);
        },
        fail: (err) => {
